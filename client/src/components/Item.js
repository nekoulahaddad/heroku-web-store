...
class Item extends Component {
  ...
  render() {
    ...
    const img1 = "";
    const img4 =
      "https://www.cobdoglaps.sa.edu.au/wp-content/uploads/2017/11/placeholder-profile.jpg";
    return (
      <div>
              ...
              <div className="ml-3 mr-3 mt-3">
                <Row>
                  <Col md={3}>
                    {!item.images ? (
                      <div>
                        ...
                        <img
                          alt="item"
                          className="img-thumbnail img_item"
                  ...
                  <Col md={6}>
                    ...
                    <span>
                      <i
                        className="fa fa-comments  batata ml-3"
                        aria-hidden="true"
                      >
                        {item.comment_count}
                      </i>
                    ...
                    <div>
                      ...
                      item.writer === this.state.user_id ? (
                        <Button
                ...
                {item.comment ? (
                  <div>
                    {item.comment.map((comment) => (
                      <div
                        className="ml-1 pb-2 border-bottom mb-2"
                        key={comment._id}
                      >
                        {!comment.user_image ? (
                          <img
                          ...
                          <span>
                            ...
                            <span>
                              ...
                              <img
                                alt="user"
                                className="image_comment rounded-circle"
                                src={`${img1}${comment.user_image}`}
                              />{" "}
                            </span>
                          </span>
                        )}
                        <span className="font-weight-bold">{comment.user}</span>{" "}
                        {comment.content}
                        <div className="small">
                          <span className="font-weight-light">
                            <ReactTimeAgo date={comment.date} locale="en" />
                          </span>
                          <span className="btn-link ml-3">
                            {comment.reply_count} replies
                          </span>
                          <i
                            onClick={this.ShowAndHide.bind(this, comment._id)}
                            className="btn-link ml-3"
                          >
                            add reply
                          </i>
                          <i
                            onClick={this.ShowAndHideReply.bind(
                              this,
                              comment._id
                            )}
                            className="btn-link ml-3"
                          ...
                          {comment.replies ? (
                            <div className="mt-2 ml-3 smaller">
                              {comment.replies.map((reply, index) => (
                                <div
                                  key={index}
                                  className="ml-1 pb-1 border-bottom mb-1"
                                >
                                  {!reply.user_image ? (
                                    <img
                                      alt="user"
