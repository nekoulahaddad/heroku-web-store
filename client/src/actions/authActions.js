...
export const updateUser = ({ name, lastname, email, images }) => (
  dispatch,
  getState
) => {
  const body = JSON.stringify({ name, lastname, email, images });

  axios
    .put("/users/updateUser", body, tokenConfig(getState))
    .then((res) => {
      dispatch({
        type: UPDATE_USER,
        payload: res.data,
...
export const changePassword = ({ oldPassword, newPassword }) => (
  ...
  axios
    .put("/users/changePassword", body, tokenConfig(getState))
    .then((res) => {
...
export const addToCart = (_id) => (dispatch, getState) => {
  axios
    .get(`/users/addToCart?productId=${_id}`, tokenConfig(getState))
    .then((res) => {
    ...
    .catch((err) => {
      dispatch(
        returnErrors(err.response.data, err.response.status, "ADD_TO_CART_FAIL")
      );
...
export const removeOneFromCart = (_id) => (dispatch, getState) => {
  axios
    .get(`/users/removeOneFromCart?productId=${_id}`, tokenConfig(getState))
    .then((res) => {
      dispatch({ type: REMOVE_ONE_FROM_CART, payload: res.data });
      //history.push('/');
    })
    .catch((err) => {
      dispatch(
...
export const removeFromCart = (_id) => (dispatch, getState) => {
  axios
    .get(`/users/removeFromCart?productId=${_id}`, tokenConfig(getState))
    .then((res) => {
...
export const userCartInfo = () => (dispatch, getState) => {
  dispatch({ type: USER_LOADING });
  axios
    .get("/users/userCartInfo", tokenConfig(getState))
    .then((res) => {
...
export const successBuy = (variables) => (dispatch, getState) => {
  axios
    .post("/users/successBuy", variables, tokenConfig(getState))
    .then((res) => {
      res.data.cart.forEach((cartItem) => {
        res.data.cartDetail.forEach((cartDetail, i) => {
          if (cartItem.id === cartDetail._id) {
    ...
    .catch((err) => {
      dispatch(
        returnErrors(err.response.data, err.response.status, "SUCCESS_BUY_FAIL")
      );
